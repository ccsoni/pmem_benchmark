...
// __PMEM_MEMCPY__ : the D2P- and P2D-copy operations are performed with pmem_memcpy() and memcpy(), respectively.
...
#ifdef __PMEM__
#define DRAM_DF_vel(ivx, ivy, ivz) (df_dram[(ivz) + NMESH_VZ * ((ivy) + NMESH_VY * (ivx))])
#endif
...
#pragma omp parallel for collapse(3) schedule(auto)
  for(int32_t ix = 0; ix < NMESH_X; ix++) {
...
#else
	      DF(ix, iy, iz).vel_grid[ivx][ivy][ivz] = (float)(im) * (float)(ivm);
#endif
	    }
	  }
	}
	...
	pmem_memcpy(DF(ix, iy, iz).vel_grid, dmem_buf, sizeof(float) * NMESH_VEL, PMEM_F_MEM_NONTEMPORAL);
#elif __PMEM_LOOP__
	for(int32_t ivx = 0; ivx < NMESH_VX; ivx++) {
	  for(int32_t ivy = 0; ivy < NMESH_VY; ivy++) {
	    for(int32_t ivz = 0; ivz < NMESH_VZ; ivz++) {
...
#pragma omp for schedule(auto) collapse(3)
    for(int32_t ix = 0; ix < NMESH_X; ix++){
      for(int32_t iy = 0; iy < NMESH_Y; iy++){
		...
		DF(ix, iy, iz).vel_grid[ivx][ivy][ivz] = df_cpy[ivx_wp];
	      }
  ...
  // loop 4 w/o SIMD integrate_vx 3loop copy
	...
	for(int32_t iz = 0; iz < NMESH_Z; iz++) {
#ifdef __PMEM__
#ifdef __PMEM_MEMCPY__
	  float *df_dram = aligned_alloc(ALIGN_SIZE, sizeof(float) * NMESH_VEL);
	  memcpy(df_dram, DF(ix, iy, iz).vel_grid, sizeof(float) * NMESH_VEL);
#elif __PMEM_LOOP__
	  float *df_dram = aligned_alloc(ALIGN_SIZE, sizeof(float) * NMESH_VEL);
	  for(int32_t ivy = vy_start; ivy < vy_end; ivy++) {
	    for(int32_t ivz = vz_start; ivz < vz_end; ivz++) {
	      for(int32_t ivx = 0; ivx < NMESH_VX; ivx++) {
		int32_t ivm = ivz + NMESH_VZ * (ivy + NMESH_VY * ivx);
		df_dram[ivm] = DF(ix, iy, iz).vel_grid[ivx][ivy][ivz];
	      }
	    }
	  }
#endif
	  ...
	  for(int32_t ivy = vy_start; ivy < vy_end; ivy++) {
		...
		df_cpy[ipadd + NMESH_VX + NPADD] = df_cpy[NMESH_VX + NPADD - 1];
	      ...
	      // /* Semi-Lagrange scheme */
	      // calc_flux_vel(df_cpy, flux, &ta);
	      // check_positivity_vel(df_check, df_tmp, df_cpy, flux, &ta);
		...
		// DF(ix, iy, iz).vel_grid[ivx][ivy][ivz] = df_tmp[ivx_wp];
#ifdef __PMEM__
		DRAM_DF_vel(ivx,ivy,ivz) = df_cpy[ivx_wp];
#else
		DF(ix, iy, iz).vel_grid[ivx][ivy][ivz] = df_cpy[ivx_wp];
#endif
	      }
...
#ifdef __PMEM__
#ifdef __PMEM_MEMCPY__
	  pmem_memcpy(DF(ix, iy, iz).vel_grid, df_dram, sizeof(float) * NMESH_VEL, PMEM_F_MEM_NONTEMPORAL);
	  free(df_dram);
#elif  __PMEM_LOOP__
	  for(int32_t ivx = 0; ivx < NMESH_VX; ivx++) {	  
	    for(int32_t ivy = vy_start; ivy < vy_end; ivy++) {
	      for(int32_t ivz = vz_start; ivz < vz_end; ivz++) {
		int32_t ivm = ivz + NMESH_VZ * (ivy + NMESH_VY * ivx);
		DF(ix, iy, iz).vel_grid[ivx][ivy][ivz] = df_dram[ivm];
	      }
	    }
	  }
	  free(df_dram);
